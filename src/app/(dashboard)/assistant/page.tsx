...
import { assistantProvider } from '@/lib/assistant/mock-assistant';
...
export default function AssistantPage() {
  ...
  const handleSend = async (textToSend: string) => {
    ...
    // Simulated "thinking" delay so the response doesn't feel instant/robotic.
    // See src/lib/assistant/mock-assistant.ts — this is a rule-based keyword
    // matcher, not a real language model. Documented in docs/PROJECT_AUDIT.md.
    const thinkingDelay = 500 + Math.random() * 700;

    try {
      const reply = assistantProvider.getReply(textToSend, { name: user?.name });
  ...
  const SUGGESTED_PROMPTS = [
    ...
    { label: "What's my balance?", text: 'What is my current wallet balance?' },
    { label: 'How do I send money?', text: 'How do I send money to someone?' },
    { label: 'My KYC status', text: 'What is my KYC verification status?' },
    { label: 'My limits & fees', text: 'What are my transaction limits and fees?' },
  ];
