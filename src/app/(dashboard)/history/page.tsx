...
/**
 * Real transaction history, backed by GET /api/transactions (fixed during
 * a security-audit pass — was previously public/unauthenticated and
 * returned hardcoded mock data). The mapping logic below previously
 * expected an entirely fictional shape (`t.sender.user`, `t.recipientWallet.user`,
 * `t.metadata.note`) that never matched any real or even mock data in
 * this project — replaced with the real WalletTransactionDoc fields.
 */
...
const TYPE_TO_TAB: Record<string, Transaction['type']> = {
  topup: 'Received', transfer_in: 'Received',
  withdrawal: 'Sent', transfer_out: 'Sent',
  bill: 'Bill', airtime: 'Airtime',
};

function formatTxDateTime(createdAt: any): { date: string; time: string } {
...
export default function HistoryPage() {
      ...
      api.get('/transactions?limit=100').then(res => {
        ...
        if (Array.isArray(data)) {
          const mapped: Transaction[] = data.map((t: any) => {
            const { date, time } = formatTxDateTime(t.createdAt);
            const isCredit = CREDIT_TYPES.has(t.type);
            return {
              id: t.id,
              ref: t.ref || t.id.slice(0, 8).toUpperCase(),
              name: t.counterpartyName || t.note || TYPE_TO_TAB[t.type] || t.type,
              date,
              time,
              amount: isCredit ? Number(t.amount) : -Number(t.amount),
              type: TYPE_TO_TAB[t.type] || 'Sent',
              status: t.status === 'successful' ? 'completed' : t.status === 'failed' ? 'failed' : 'pending',
              method: t.type === 'bill' ? 'Bill Payment' : t.type === 'airtime' ? 'Airtime' : 'GhanaPay Wallet',
              fee: t.fee || 0,
              note: t.note || '',
              phone: '', // not tracked per-transaction in the real ledger — kept for type compatibility only
            };
          });
  ...
  return (
    <PageWrap
      ...
      action={
        <Btn
          ...
          onClick={() => {
            const header = ['Date', 'Time', 'Name', 'Type', 'Reference', 'Amount', 'Fee', 'Status', 'Note'];
            const lines = filtered.map(t =>
              [t.date, t.time, t.name, t.type, t.ref, t.amount, t.fee, t.status, (t.note || '').replace(/,/g, ';')].join(',')
            );
    ...
    >
      {error && (
        <div className="p-4 rounded-xl bg-error-container text-error text-sm mb-6">{error}</div>
      )}
