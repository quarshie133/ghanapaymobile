...
function formatTxDate(createdAt: any): string {
  ...
  return new Date(seconds * 1000).toLocaleDateString('en-US', { day: 'numeric', month: 'short' });
}
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <div className="w-9 h-9 rounded-full bg-primary-fixed text-primary font-bold flex items-center justify-center shrink-0 text-xs">
                          {getInitials(tx.counterpartyName || tx.note || 'Transaction')}
                        </div>
                        <div className="min-w-0">
                          <p className="text-sm font-bold text-primary truncate max-w-[130px] sm:max-w-none">{tx.counterpartyName || tx.note || 'Transaction'}</p>
                          <p className="text-[10px] sm:text-[11px] text-secondary truncate max-w-[130px] sm:max-w-none">{formatTxDate(tx.createdAt)} · {tx.note || tx.type}</p>
                        </div>
                      </div>
                      <div className="text-right">
                        <p className={`text-sm font-bold ${isCreditType(tx.type) ? 'text-success' : 'text-error'}`}>
                          {isCreditType(tx.type) ? '+' : '-'}{formatCurrency(Math.abs(tx.amount))}
                        </p>
                        <Badge
                          label={tx.status}
                          type={tx.status === 'successful' ? 'success' : tx.status === 'failed' ? 'error' : 'warning'}
                        />
                      </div>
                    </div>
                  ))
                ...
                recent.map((tx, idx) => (
                  ...
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="text-sm font-bold text-primary">{tx.counterpartyName || tx.note || 'Transaction'}</p>
                        <p className="text-[11px] text-secondary">{formatTxDate(tx.createdAt)} • {tx.note || tx.type}</p>
                      </div>
                    </div>
                    <div className="text-right">
                      <p className={`text-sm font-bold ${isCreditType(tx.type) ? 'text-success' : 'text-error'}`}>
